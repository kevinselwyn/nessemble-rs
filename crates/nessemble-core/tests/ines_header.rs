...
//! extensions (`.inesbat`, `.ines4scr`, `.inesprgram`, `.inestv`). These fields
//! are not produced by the reference v1.1.1 binary, so they are checked here by
//! asserting the emitted 16-byte header directly rather than via a golden ROM.
...
#[test]
fn extensions_leave_bytes_10_to_15_zero() {
    // Setting every Tier 1 field must not disturb the trailing padding, so the
    // file is not misdetected as NES 2.0 or corrupt.
    let h = header(
        ".inesprg 1\n.ineschr 1\n.inesbat 1\n.ines4scr 1\n.inesprgram 8\n.inestv 1\n",
    );
    assert_eq!(&h[10..16], &[0; 6]);
}
