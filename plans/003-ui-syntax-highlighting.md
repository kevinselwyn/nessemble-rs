...
Give the in-browser `<nessemble-assembler>` editor **live syntax highlighting**
as the user types, colored by the *same* tokenizer the assembler and the Language
Server already use — so highlighting can never drift from a second, parallel
grammar.
...
- **C — a static grammar (TextMate/Prism/highlight.js) or build-time pre-render.**
  A hand-written regex grammar is a *second source of truth* that drifts from the
  real lexer and handles nessemble's quirks poorly (mnemonic-vs-label, the `<`
  zero-page prefix, `[ ]` indirect addressing, `.dw`/directive families).
  Build-time pre-rendering only works for *fixed* code (docs blocks), not an
  interactive editor. Useful for **other** surfaces (mdBook code blocks; GitHub
  Linguist requires a TextMate grammar) but not for this one. **Out of scope
  here** — see §8.
...
parity stays **122/122** throughout; the only Rust behavior touched is a
refactor of the LSP's semantic-tokens *source*, guarded by its existing tests.
...
### Phase 3 — Theming + docs/site integration
- Theme-aware palette across mdBook themes and the marketing site.
- **Done when:** a local `xtask dist` produces a `site/` where the docs' and
  homepage's editors are highlighted and legible in light and dark (headless
...
- **Versioning** — land the feature under a pre-release `-dev` version (as the LSP
  ...
  Phase 4. Main is currently `2.7.0`; a minor feature → `2.8.0-dev` while landing,
  `2.8.0` to ship.
...
- A TextMate/Prism grammar or GitHub Linguist highlighting — a *separate* artifact
  for a *different* surface (GitHub, generic docs blocks); may be a later plan, and
  is best generated from the same lexer to avoid drift.
